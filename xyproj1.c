...
void check_mouse(XEvent *e)
{
    ...
    if (e->type == ButtonPress) {
        ...
        if (e->xbutton.button==3) {
            int mx = e->xbutton.x;
            int my = e->xbutton.y;
            int inside = (mx > box.x && mx < box.x + box.w) && (my > box.y && my < box.y + box.h);
...
//function that handles key inputs on keyboard
int check_keys(XEvent *e)
{
    int key;
...
//function to write text on windows
...
void render(void)
{
    ...
    if (child == 0) {
        drawText(10, y, "This is Project Phase-1");
        y+=12;
        drawText(10, y, "Parent window");
        y+=12;
        drawText(10, y, "Press C to spawn child window");
        y+=12;
        drawText(10, y, "Drag box with left mouse button");
        y+=12;
        drawText(10, y, "Right-click inside box to close the child.");
        y+=12;
    }
    if (child == 1) {
        drawText(10, 15, "This is Project Phase-1");
    ...
    //fill box if mouse is hovering over parent
    if (box.hover == 1 && child) {
